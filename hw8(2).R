...
#fit=optimize(f,seq(0,0.5,0.0001))
#fit$minimum
...
for (trial in 1:10000){ 
  ...
  tau=-log(1-u)/0.02
  for (i in 1:80){
    if (tau[i]<=1) {loss[trial]=loss[trial]+10*(1-rbeta(1,2,2))}
  }
  for (i in 81:100){
    if (tau[i]<=1) {loss[trial]=loss[trial]+20*(1-rbeta(1,2,2))}
  }
}
hist(loss,main = "Histogram of loss with recovery")
mean(loss)
##(b)
...
hist(loss,breaks=100,main = "Histogram of loss with recovery and copula correlation",xlim = c(0,150))
mean(loss)
