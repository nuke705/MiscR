...
library(plyr)
setwd("E:/study")
ndata<-read_excel("valueonly.xlsx")
pretotal=NA
cutoff=matrix(data=0,nrow=48,ncol=4)
for (i in 1:48) {
  ndata_train = ndata[(1+4*(i-1)):(384 + 4*(i-1)),]
  ndata_test = ndata[(385+4*(i-1)):(388+4*(i-1)),]
  tc <- rpart.control(minsplit=50,minbucket=384/13,maxdepth=10,xval=5,cp=0.005)
  tree=rpart(`Next 1 W Index Return`~`Factor 1` + buf+HiborD+ESI + PMI,data=ndata_train,control=tc)
  #summary(tree)
